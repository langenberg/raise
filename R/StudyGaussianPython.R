...
#'
StudyGaussianPython <- R6::R6Class(
    "StudyGaussianPython",
    inherit = StudyGaussian,
    private = list(
    ),
    active = list(
    ),
    public = list(
        ...
        get_model_summary_md_force = function(...) {
            to_be_run <- "
import statsmodels.formula.api as sm  # Import statsmodel library which includes simple linear regression (ols)
import pandas as pd
import numpy as np
...
model1 = sm.ols(formula=\"##formula##\", data=df).fit()
