...
binary_outcome <- StudyBinaryPython$new()$generate(
...
binary_outcome$sample(n = 5000, empirical = T)
binary_outcome$export(output_file = "summary_binary.html", output_dir = "~/Downloads")

# binary_outcome$cover_tracks()
...
# 
# study_summary <- SummaryStudyBinary$new(study_design = binary_outcome, data = dat_binary)
# study_summary$render(output_file = "summary_binary.html", output_dir = "~/Downloads")


# 3. continuous outcome ---------------------------------------------------

continuous_outcome <- StudyGaussian$new()$generate(
    description = description,
    n_continuous = n_continuous,
    n_categorical = n_categorical,
    n_binary = n_binary
